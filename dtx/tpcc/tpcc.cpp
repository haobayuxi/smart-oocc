...
void TPCC::LoadTable(node_id_t node_id, node_id_t num_server,
  ...
  printf(
      ...
      "sizeof(tpcc_customer_val_t) = %lu, sizeof(tpcc_customer_index_val_t) = "
      "%lu\n"
      ...
      sizeof(tpcc_customer_val_t), sizeof(tpcc_customer_index_val_t),
  ...
  if (BACKUP_DEGREE < num_server) {
    for (node_id_t i = 1; i <= BACKUP_DEGREE; i++) {
      if ((node_id_t)TPCCTableType::kWarehouseTable % num_server ==
      ...
      if ((node_id_t)TPCCTableType::kDistrictTable % num_server ==
          (node_id - i + num_server) % num_server) {
        ...
        std::string district_config_filepath =
            "workload/tpcc/tpcc_tables/district.json";
        auto district_json_config =
            JsonConfig::load_file(district_config_filepath);
        auto district_table_config = district_json_config.get("table");
        district_table =
            new HashStore((table_id_t)TPCCTableType::kDistrictTable,
                          warehouse_table_config.get("bkt_num").get_uint64() *
      ...
      if ((node_id_t)TPCCTableType::kCustomerTable % num_server ==
          (node_id - i + num_server) % num_server) {
        ...
        std::string customer_config_filepath =
            "workload/tpcc/tpcc_tables/customer.json";
        auto customer_json_config =
            JsonConfig::load_file(customer_config_filepath);
        auto customer_table_config = customer_json_config.get("table");
      ...
      if ((node_id_t)TPCCTableType::kOrderTable % num_server ==
          (node_id - i + num_server) % num_server) {
        printf(
            "Backup: Initializing Order+OrderIndex+NewOrder+OrderLine table\n");
        std::string warehouse_config_filepath =
            "workload/tpcc/tpcc_tables/warehouse.json";
        auto warehouse_json_config =
            JsonConfig::load_file(warehouse_config_filepath);
      ...
      if ((node_id_t)TPCCTableType::kStockTable % num_server ==
          (node_id - i + num_server) % num_server) {
      ...
      if ((node_id_t)TPCCTableType::kItemTable % num_server ==
          (node_id - i + num_server) % num_server) {
...
void TPCC::PopulateWarehouseTable(
  ...
  for (uint32_t w_id = 1; w_id <= num_warehouse; w_id++) {
    ...
    total_warehouse_records_inserted += LoadRecord(
        warehouse_table, warehouse_key.item_key, (void*)&warehouse_val,
        sizeof(tpcc_warehouse_val_t),
        (table_id_t)TPCCTableType::kWarehouseTable, mem_store_reserve_param);
    total_warehouse_records_examined++;
  }
  // printf("total_warehouse_records_inserted = %d,
  // total_warehouse_records_examined = %d\n",
  //        total_warehouse_records_inserted, total_warehouse_records_examined);
}
void TPCC::PopulateDistrictTable(
    unsigned long seed, MemStoreReserveParam* mem_store_reserve_param) {
  ...
  for (uint32_t w_id = 1; w_id <= num_warehouse; w_id++) {
    for (uint32_t d_id = 1; d_id <= num_district_per_warehouse; d_id++) {
      ...
      total_district_records_inserted += LoadRecord(
          district_table, district_key.item_key, (void*)&district_val,
          sizeof(tpcc_district_val_t),
          (table_id_t)TPCCTableType::kDistrictTable, mem_store_reserve_param);
      total_district_records_examined++;
    }
  }
  // printf("total_district_records_inserted = %d,
  // total_district_records_examined = %d\n",
  //        total_district_records_inserted, total_district_records_examined);
}

// no batch in this implementation
void TPCC::PopulateCustomerAndHistoryTable(
    unsigned long seed, MemStoreReserveParam* mem_store_reserve_param) {
  ...
  FastRandom random_generator(seed);
  printf(
      "num_warehouse = %d, num_district_per_warehouse = %d, "
  ...
  for (uint32_t w_id = 1; w_id <= num_warehouse; w_id++) {
    for (uint32_t d_id = 1; d_id <= num_district_per_warehouse; d_id++) {
      for (uint32_t c_id = 1; c_id <= num_customer_per_district; c_id++) {
        ...
        total_customer_records_inserted += LoadRecord(
            customer_table, customer_key.item_key, (void*)&customer_val,
            sizeof(tpcc_customer_val_t),
            (table_id_t)TPCCTableType::kCustomerTable, mem_store_reserve_param);
        ...
        DataItem* mn =
            GetRecord(customer_index_table, customer_index_key.item_key,
                      (table_id_t)TPCCTableType::kCustomerIndexTable);
        assert(mn == NULL);
        if (mn == NULL) {
          customer_index_val.c_id = customer_key.c_id;
          total_customer_index_records_inserted += LoadRecord(
              customer_index_table, customer_index_key.item_key,
              (void*)&customer_index_val, sizeof(tpcc_customer_index_val_t),
              (table_id_t)TPCCTableType::kCustomerIndexTable,
              mem_store_reserve_param);
        ...
        total_history_records_inserted += LoadRecord(
            history_table, history_key.item_key, (void*)&history_val,
            sizeof(tpcc_history_val_t),
            (table_id_t)TPCCTableType::kHistoryTable, mem_store_reserve_param);
...
void TPCC::PopulateOrderNewOrderAndOrderLineTable(
    unsigned long seed, MemStoreReserveParam* mem_store_reserve_param) {
    ...
    for (uint32_t d_id = 1; d_id <= num_district_per_warehouse; d_id++) {
      ...
      for (uint32_t c = 1; c <= num_customer_per_district; c++) {
        ...
        total_order_records_inserted += LoadRecord(
            order_table, order_key.item_key, (void*)&order_val,
            sizeof(tpcc_order_val_t), (table_id_t)TPCCTableType::kOrderTable,
            mem_store_reserve_param);
        ...
        DataItem* mn = GetRecord(order_index_table, order_index_key.item_key,
                                 (table_id_t)TPCCTableType::kOrderIndexTable);
        assert(mn == NULL);
        if (mn == NULL) {
          order_index_val.o_id = order_key.o_id;
          order_index_val.debug_magic = tpcc_add_magic;
          total_order_index_records_inserted += LoadRecord(
              order_index_table, order_index_key.item_key,
              (void*)&order_index_val, sizeof(tpcc_order_index_val_t),
              (table_id_t)TPCCTableType::kOrderIndexTable,
              mem_store_reserve_param);
        ...
        if (c >
          ...
          total_new_order_records_inserted +=
              LoadRecord(new_order_table, new_order_key.item_key,
                         (void*)&new_order_val, sizeof(tpcc_new_order_val_t),
                         (table_id_t)TPCCTableType::kNewOrderTable,
                         mem_store_reserve_param);
        ...
        for (uint32_t l = 1; l <= uint32_t(order_val.o_ol_cnt); l++) {
          ...
          total_order_line_records_inserted +=
              LoadRecord(order_line_table, order_line_key.item_key,
                         (void*)&order_line_val, sizeof(tpcc_order_line_val_t),
                         (table_id_t)TPCCTableType::kOrderLineTable,
                         mem_store_reserve_param);
  ...
  RDMA_LOG(INFO) << "total_order_records_inserted = "
                 << total_order_records_inserted
                 << ", total_order_records_examined = "
                 << total_order_records_examined;
...
void TPCC::PopulateItemTable(unsigned long seed,
  ...
  for (int64_t i_id = 1; i_id <= num_item; i_id++) {
    ...
    total_item_records_inserted += LoadRecord(
        item_table, item_key.item_key, (void*)&item_val,
        sizeof(tpcc_item_val_t), (table_id_t)TPCCTableType::kItemTable,
        mem_store_reserve_param);
  ...
  for (uint32_t w_id = 1; w_id <= num_warehouse; w_id++) {
    for (uint32_t i_id = 1; i_id <= num_item; i_id++) {
      ...
      total_stock_records_inserted += LoadRecord(
          stock_table, stock_key.item_key, (void*)&stock_val,
          sizeof(tpcc_stock_val_t), (table_id_t)TPCCTableType::kStockTable,
          mem_store_reserve_param);
...
int TPCC::LoadRecord(HashStore* table, itemkey_t item_key, void* val_ptr,
...
DataItem* TPCC::GetRecord(HashStore* table, itemkey_t item_key,
                          table_id_t table_id) {
  /* get from HashStore */
