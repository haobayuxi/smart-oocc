...
#define FREQUENCY_UPDATE_SUBSCRIBER_DATA 6  // Single
...
void TATP::LoadTable(MemStoreAllocParam *mem_store_alloc_param,
  ...
  subscriber_table = new HashStore((table_id_t)TATPTableType::kSubscriberTable,
                                   SubscriberItemCount, mem_store_alloc_param);
  ...
  access_info_table = new HashStore((table_id_t)TATPTableType::kAccessInfoTable,
                                    AccessInfoItemCount, mem_store_alloc_param);
...
void TATP::PopulateAccessInfoTable(
  ...
  /* Populate the table */
  for (uint32_t s_id = 0; s_id < subscriber_size; s_id++) {
    std::vector<uint8_t> ai_type_vec =
...
void TATP::PopulateSpecfacAndCallfwdTable(
  ...
  /* Populate the tables */
  for (uint32_t s_id = 0; s_id < subscriber_size; s_id++) {
    std::vector<uint8_t> sf_type_vec =
