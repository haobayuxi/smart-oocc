...
class DTXContext {
 public:
  ...
  char *Alloc(size_t size) {
    return tl_data_[GetThreadID()].buf_alloc->Alloc(size);
  }
  ...
  offset_t GetNextLogOffset(node_id_t node_id, size_t log_size) {
    return log_base_[node_id] +
           tl_data_[GetThreadID()].log_alloc->GetNextLogOffset(node_id,
                                                               log_size);
  }
  ...
  void BeginTask() { tl_data_[GetThreadID()].backoff.begin_task(); }

  void EndTask() { tl_data_[GetThreadID()].backoff.end_task(); }
