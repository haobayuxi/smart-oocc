...
struct DataItem {
  ...
  uint8_t
      user_insert;  // 1: User insert operation, 0: Not user insert operation
  ...
  ALWAYS_INLINE size_t GetSerializeSize() const { return sizeof(*this); }

  ALWAYS_INLINE void Serialize(char *undo_buffer) {
    memcpy(undo_buffer, (char *)this, sizeof(*this));
  }

  ALWAYS_INLINE uint64_t GetRemoteLockAddr() { return remote_offset; }
...
const static size_t RFlushReadSize =
    1;  // The size of RDMA read, that is after write to emulate rdma flush
...
struct HashMeta {
  ...
  HashMeta(table_id_t table_id, uint64_t data_ptr, uint64_t bucket_num,
           size_t node_size, offset_t base_off)
      : table_id(table_id),
        data_ptr(data_ptr),
        base_off(base_off),
...
class HashStore {
 public:
  ...
  HashStore(table_id_t table_id, uint64_t bucket_num, MemStoreAllocParam *param)
    ...
    assert((uint64_t)param->mem_store_start + param->mem_store_alloc_offset +
               table_size <=
  ...
  DataItem *LocalInsert(itemkey_t key, const DataItem &data_item,
                        MemStoreReserveParam *param);

  DataItem *LocalPut(itemkey_t key, const DataItem &data_item,
                     MemStoreReserveParam *param);
...
ALWAYS_INLINE
DataItem *HashStore::LocalInsert(itemkey_t key, const DataItem &data_item,
                                 MemStoreReserveParam *param) {
  uint64_t hash = GetHash(key);
  auto *node = (HashNode *)(hash * sizeof(HashNode) + data_ptr);
  ...
  auto *new_node =
      (HashNode *)(param->mem_store_reserve + param->mem_store_reserve_offset);
...
ALWAYS_INLINE
DataItem *HashStore::LocalPut(itemkey_t key, const DataItem &data_item,
                              MemStoreReserveParam *param) {
  DataItem *res;
